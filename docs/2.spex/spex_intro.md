...
2) Most people without the disease will have low values (normally distribution, but very tight) of pathology, while people on their way to the disease will have high values of pathology (normally distributed, but very spread out) [^Vogel_2020],[^Mormino_2014],[^Ozlen_2022]
...
Below are a few definitions for terms I will reuse across the documentation. Note that the definitions here might be a bit broad, as the **spatial extent** can be calculated in many different ways, which come with their own definition. {ref}`More info in the tutorial page <2.spex/spex_module:Spatial extent analysis>`.
...
The spatial extent was developed with Alzheimer's disease pathology (amyloid and tau) as measured with positron emission tomography; but any set of continuous brain measures (e.g., atrophy, thickness, connectivity) that **can be binarized or clustered** should be able to work with the spatial extent.

`sihnpy` specifically needs a spreadsheet imported into Python (i.e., a `pandas.DataFrame`), where each row is a participant and each column is a brain region.

## Use cases and limitations

The **spatial extent** is a very flexible approach that can be adapted to include many different methodologies to determine "abnormality". As such, the use cases and limitations highly depend on the methodology chosen. For methodology-specific drawbacks, please look {ref}`at the tutorial <2.spex/spex_module:Spatial extent analysis>` for each of the methods currently implemented in `sihnpy`.

Some general conditions to use the **spatial extent** are:
* Have more than 1 brain region under study (note that you can still use the module to derive thresholds for single regions)
